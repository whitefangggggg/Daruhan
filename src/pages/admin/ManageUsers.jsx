...
const ROLE_CONFIG = {
  admin: {
    badge: 'bg-violet-100/80 dark:bg-violet-900/25 text-violet-700 dark:text-violet-400 border-violet-200/70 dark:border-violet-800/50',
    dot: 'bg-violet-500',
    label: 'Admin',
    icon: Shield,
  },
  user: {
    badge: 'bg-gray-100/70 dark:bg-slate-800 text-gray-500 dark:text-gray-400 border-gray-200/70 dark:border-slate-700',
    dot: 'bg-gray-400',
    label: 'Player',
    icon: User,
  },
...
export default function ManageUsers() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-6 sm:py-8 lg:py-10 pb-[max(2rem,env(safe-area-inset-bottom))]">
        ...
        >
          <div className="px-5 py-3 border-b border-brand-gold-200/60 dark:border-brand-navy-700/50 bg-brand-gold-50/40 dark:bg-brand-navy-900/20">
                  ...
                  >
                    <div className="min-w-0 flex items-center gap-3">
                      <div className={`w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0 ${
                        profile.role === 'admin' ? 'bg-violet-100/70 dark:bg-violet-900/25' : 'bg-brand-gold-100/60 dark:bg-brand-navy-900/30'
                      }`}>
                        <RoleIcon
                          size={15}
                          className={profile.role === 'admin' ? 'text-violet-600 dark:text-violet-400' : 'text-brand-gold-600 dark:text-brand-gold-400'}
                        />
                      </div>
                      <div className="min-w-0">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {!profile.onboarding_completed && (
                            <span className="text-[10px] font-semibold text-amber-700 dark:text-amber-400 bg-amber-50 dark:bg-amber-900/20 border border-amber-200/70 dark:border-amber-900/40 px-1.5 py-0.5 rounded-full flex-shrink-0">
                              Onboarding
                            </span>
                          )}
                        </div>
